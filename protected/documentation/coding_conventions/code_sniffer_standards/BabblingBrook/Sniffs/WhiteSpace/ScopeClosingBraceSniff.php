<?php
/**
 * BabblingBrook_Sniffs_WhiteSpace_ScopeClosingBraceSniff.
 *
 * PHP version 5
 *
 * @category  PHP
 ...
 * @link      http://pear.php.net/package/PHP_CodeSniffer
 */
class BabblingBrook_Sniffs_WhiteSpace_ScopeClosingBraceSniff implements PHP_CodeSniffer_Sniff
{
    ...
    {	
		...
		// Don't error if there is an open tag on the same line. 
		// The brace is insde some nested html.	
		if($tokens[$lineStart + 1]['code'] === T_OPEN_TAG || $tokens[$lastContent]['code'] === T_OPEN_TAG) {
			return;
		}
		...
		// Break statments are indented with the code.
		if ($tokens[$scopeEnd]['code'] === T_BREAK) {
			$braceIndent -= 4;
		}
		
		// Default openers can end with any statment and they are inline with the code.
