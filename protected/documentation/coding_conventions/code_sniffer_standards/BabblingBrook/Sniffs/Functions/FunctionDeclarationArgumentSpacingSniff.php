<?php
/**
 * BabblingBrook_Sniffs_Functions_FunctionDeclarationArgumentSpacingSniff.
 *
 * PHP version 5
 *
 * @category  PHP
 ...
 * @link      http://pear.php.net/package/PHP_CodeSniffer
 */
class BabblingBrook_Sniffs_Functions_FunctionDeclarationArgumentSpacingSniff implements PHP_CodeSniffer_Sniff
{
...
//print_r(($tokens[$functionName]['column'] + strlen($tokens[$functionName]['content']) + 1) . " | " . $tokens[$openBracket]['column'] . "\n");
		// Check that there is one space between the funciton name and the opening parenthesis.
		$end_of_name = $tokens[$functionName]['column'] + strlen($tokens[$functionName]['content']);
		$open_bracket_column = $tokens[$openBracket]['column'];
		if($end_of_name !== $open_bracket_column) {
			$error = 'Expected 0 spaces between funciton name and the opening paramater parentesis. %s found';
			$data  = array($open_bracket_column - $end_of_name);
			$phpcsFile->addError($error, $openBracket, 'SpaceAfterFuncitonName', $data);
		}
