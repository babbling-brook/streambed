<?php
/**
 * BabblingBrook_Sniffs_Functions_FunctionCallSignatureSniff.
 *
 * PHP version 5
 *
 * @category  PHP
 ...
 * @link      http://pear.php.net/package/PHP_CodeSniffer
 */
class BabblingBrook_Sniffs_Functions_FunctionCallSignatureSniff implements PHP_CodeSniffer_Sniff
{
    ...
    public function processMultiLineCall(PHP_CodeSniffer_File $phpcsFile, $stackPtr, $openBracket, $tokens)
    {
        ...
        for ($i = ($openBracket + 1); $i < $closeBracket; $i++) {
            ...
            if ($tokens[$i]['line'] !== $lastLine) {
				...
				// Ignore multiline string concatinations
				if($tokens[$i]['code'] === T_WHITESPACE && $tokens[$i + 1]['code'] === T_STRING_CONCAT) {
					continue;
				}
